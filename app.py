...
# ---------- 초기 설정 ----------
# 한글 폰트 관련 설정 코드는 제거하거나 주석 처리합니다.
plt.rcParams['axes.unicode_minus'] = False # 마이너스 기호 깨짐 방지 (이것은 유지)
...
base_stats_initial = {"인내력": 6, "충성심": 6, "속도": 6, "체력": 6}
main_stat_initial = 14
...
def get_specialty_bonus_for_stage(specialty_type, stage):
    ...
    if specialty_type in specialty_rewards_by_type_and_stage:
        if stage in specialty_rewards_by_type_and_stage[specialty_type]:
            ...
            # --- 이곳이 수정되었습니다: rewards_at_add -> rewards_at_stage.items()
...
d_stat = d_stat_map[category] # 주 스탯
remaining_stats = [s for s in stat_order if s != d_stat] # 이전 수정된 부분: s for s s -> s for s
a_stat, b_stat, c_stat = remaining_stats # 기타 스탯
...
if st.session_state["calculated"]:
    ...
    specialty_inputs = {
        "노비스 에너지": novice_energy_stage, "노비스 터내서티": novice_tenacity_stage,
        "노비스 링크리지": novice_linkage_stage, "노비스 래피드": novice_rapid_stage,
        "비기너 에너지": beginner_energy_stage, "비기너 터내서티": beginner_tenacity_stage,
        "비기너 링크리지": beginner_linkage_stage, "비기너 래피드": beginner_rapid_stage,
        "레이즈 에너지": raise_energy_stage, "레이즈 터내서티": raise_tenacity_stage,
        "레이즈 링크리지": raise_linkage_stage, "레이즈 래피드": raise_rapid_stage
    }
    ...
    for stat_name in stat_order:
        if exclude_hp and stat_name == "체력":
            continue
        if stat_name == a_stat: total_sim_pure += simulated_pure_a
        elif stat_name == b_stat: total_sim_pure += simulated_pure_b
        elif stat_name == c_stat: total_sim_pure += simulated_pure_c
        elif stat_name == d_stat: total_sim_pure += simulated_pure_d

    total_percentile = np.sum(total_sim_pure > user_total_pure) / num_sim * 100

    # 개별 스탯 백분위 및 레벨당 평균 증가량
    ...
    st.info(f"\U0001F4A1 상위 약 {total_percentile:.2f}% {'(체력 제외)' if exclude_hp else ''}에 해당합니다.")
    st.markdown(f"### \U0001F43E 선택 견종: **{category}** / 펫 레벨: **{level}**")

    # 스탯 요약 테이블
    df_data = {
        "스탯": [a_stat, b_stat, c_stat, d_stat],
        "입력값 (시설물/특기 포함)": [a_input, b_input, c_input, d_input],
        "순수 펫 스탯 (시설물/특기 제외)": [
            user_pure_stats[a_stat], user_pure_stats[b_stat],
            user_pure_stats[c_stat], user_pure_stats[d_stat]
        ],
        "시설물 증가량": [
            total_facility_bonuses[a_stat], total_facility_bonuses[b_stat],
            total_facility_bonuses[c_stat], total_facility_bonuses[d_stat]
        ],
        "특기 증가량": [
            total_specialty_bonuses[a_stat], total_specialty_bonuses[b_stat],
            total_specialty_bonuses[c_stat], total_specialty_bonuses[d_stat]
        ],
    ...
    # 그래프 제목, 축 라벨, 범례만 영어로 변경
    fig, ax = plt.subplots(figsize=(10, 4))
    sns.histplot(total_sim_pure, bins=50, kde=True, ax=ax, color='skyblue')
    ax.axvline(user_total_pure, color='red', linestyle='--', label='Your Pure Total') # 범례 텍스트 영어
    ax.set_title(f"Total Stat Distribution {'(Excluding Health) ' if exclude_hp else ''}(Pure Stats)") # 그래프 제목 영어
    ax.set_xlabel("Total Stats") # x축 라벨 영어
    ax.legend() # 범례 표시
    st.pyplot(fig)
    ...
    st.markdown("---")
    # ---------- 목표 스탯 입력 ----------
    calc_goal = st.checkbox("\U0001F3AF 레벨 20 목표 스탯 도달 확률 보기")

    if calc_goal:
        st.subheader("목표 스탯 입력 (레벨 20 기준 총 스탯)")
        col1, col2, col3, col4 = st.columns(4)
        target_a = col1.number_input(f"{a_stat} 목표", min_value=0, value=35, step=1)
        target_b = col2.number_input(f"{b_stat} 목표", min_value=0, value=35, step=1)
        target_c = col3.number_input(f"{c_stat} 목표", min_value=0, value=35, step=1)
        target_d = col4.number_input(f"{d_stat} 목표 (주 스탯)", min_value=0, value=100, step=1)
        ...
        if remaining_upgrades >= 0:
            ...
            st.warning("펫 레벨이 이미 20을 초과했습니다. 레벨 20 목표 시뮬레이션은 건너뜀니다.")
